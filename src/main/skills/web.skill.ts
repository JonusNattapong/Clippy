 ...
 * Web Skill
 *
 * A built-in skill that provides web search and URL fetching capabilities.
 */
 ...
 */
function getTavilyApiKey(context: SkillContext): string {
  const apiKey = context.getSetting("settings.tavilyApiKey");
  return apiKey ? String(apiKey) : process.env.TAVILY_API_KEY || "";
}

/**
 * Create the Web Skill
 */
export default function createWebSkill(): Skill {
  return {
    meta: {
      id: "web",
      name: "Web Tools",
      version: "1.0.0",
      description: "Provides web search and URL fetching capabilities",
      author: "Clippy Team",
      categories: ["web", "search", "utility"],
      keywords: ["web", "search", "url", "fetch", "internet"],
    ...
    init: async (context: SkillContext) => {
      context.log("Web skill initialized", "info");
    },

    destroy: async (context: SkillContext) => {
      context.log("Web skill destroyed", "info");
    },
  };
}
