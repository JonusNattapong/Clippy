...
async function main() {
  ...
  const apiUrl = process.env.OPENROUTER_API_URL || 'https://openrouter.ai/api/v1/images/generate';
  const tag = process.env.GITHUB_REF_NAME || process.env.GITHUB_REF || 'release';
  ...
  try {
    ...
    if (!b64) {
      console.error('No image data found in response:', JSON.stringify(json).slice(0, 1000));
      process.exit(1);
    }
